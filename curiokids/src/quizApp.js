...
const QuizApp = () => {
  ...
  // Timer logic (starts only when questions are loaded)
  useEffect(() => {
    if (!questionsLoaded) return; // Start only when questions are loaded
    if (timeLeft === 0) {
      setShowScore(true);
      return;
    }
  ...
  return (
    ...
    >
      {/* Timer (only shows when questions are loaded) */}
      {questionsLoaded && (
